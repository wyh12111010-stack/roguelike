...
from ui_theme import (
    THEME_COLORS, AnimatedBackground, draw_panel, draw_button, 
    draw_resource, draw_glow, draw_ancient_border, create_gradient_surface,
    ImmortalParticle, get_element_color
)
from config import SCREEN_WIDTH, SCREEN_HEIGHT, get_font
...
def draw_portal_enhanced(surface, rect, label, color, hover_color, is_hover, cam, sublabel=None):
    ...
    for i in range(inner_local.width // 2, 0, -2):
        ...
        pygame.draw.ellipse(portal_surface, color_with_alpha, 
                          inner_local.inflate(-2 * (inner_local.width // 2 - i), 
                                            -2 * (inner_local.height // 2 - i)))
...
def draw_player_enhanced(surface, player_rect, cam):
    ...
    pygame.draw.circle(surface, (*THEME_COLORS["immortal_cyan"], 100), 
                      (pr.centerx, pr.bottom), circle_radius, 2)
...
def draw_hud_enhanced(surface):
    ...
    gradient = create_gradient_surface(
        SCREEN_WIDTH, 60,
        (*THEME_COLORS["bg_mid"], 200),
        (*THEME_COLORS["bg_deep"], 150)
    )
    surface.blit(gradient, (0, 0))
    
    # 底部边框
    pygame.draw.line(surface, THEME_COLORS["panel_border"], 
                    (0, 59), (SCREEN_WIDTH, 59), 2)
...
def draw_selection_panel_enhanced(surface, items, choice, zone_rect, font, attr_colors, out_rects):
    ...
    for i, item in enumerate(items):
        ...
        if is_sel or is_hover:
            glow_color = attr_colors.get(item.attr, THEME_COLORS["immortal_gold"]) if attr_colors else THEME_COLORS["immortal_gold"]
        ...
        gradient = create_gradient_surface(
            item_size, item_size,
            bg_color,
            tuple(max(0, c - 30) for c in bg_color[:3])
        )
        ...
        c = get_element_color(item.attr.name) if (attr_colors and hasattr(item, 'attr')) else THEME_COLORS["text_primary"]
