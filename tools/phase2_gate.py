...
def _check_reward_weights():
    ...
    _assert(
        COMBAT_REWARD_WEIGHTS["lingshi"]
        >= COMBAT_REWARD_WEIGHTS["accessory"]
        > COMBAT_REWARD_WEIGHTS["fabao"]
        > COMBAT_REWARD_WEIGHTS["daoyun"],
...
def _check_meta_growth_costs():
    _assert(all(x < y for x, y in itertools.pairwise(ACCESSORY_SLOT_COSTS)), "饰品槽成长成本应递增")
    _assert(all(x < y for x, y in itertools.pairwise(SHOP_REFRESH_COSTS)), "商店刷新成长成本应递增")
    _assert(all(x < y for x, y in itertools.pairwise(START_ACCESSORY_COSTS)), "开局饰品成长成本应递增")
    _assert(all(x < y for x, y in itertools.pairwise(POTION_CAP_COSTS)), "丹药上限成长成本应递增")
...
def _check_currency_curve():
    ...
    _assert(
        DAOYUN_COMBAT_REWARD < DAOYUN_ELITE < min(DAOYUN_BOSS) < max(DAOYUN_BOSS) < DAOYUN_VICTORY, "道韵产出梯度应递进"
    )
