...
def get_reaction(linggen_attr: Attr, fabao_attr: Attr) -> Optional[str]:
    """
    元素反应：灵根 + 法宝 相克时的效果
    返回反应名，None 表示无反应
    """
...
def get_self_reaction(player) -> Optional[str]:
    """
...
def get_reaction_for_hit(attacker_attr: Attr, target_attr: Attr) -> Optional[str]:
    """
    攻击者属性击中目标属性时的元素反应。
...
def attr_from_str(s: str) -> Attr:
    ...
    if not s:
        return Attr.NONE
    m = {
        "metal": Attr.METAL, "jin": Attr.METAL, "金": Attr.METAL,
        "wood": Attr.WOOD, "mu": Attr.WOOD, "木": Attr.WOOD,
        "water": Attr.WATER, "shui": Attr.WATER, "水": Attr.WATER,
        "fire": Attr.FIRE, "huo": Attr.FIRE, "火": Attr.FIRE,
        "earth": Attr.EARTH, "tu": Attr.EARTH, "土": Attr.EARTH,
        "none": Attr.NONE,
    }
