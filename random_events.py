...
def upgrade_random_accessory():
    ...
    def effect(player, game):
        ...
        acc, level = random.choice(player.accessories)
        # 找到并升级
...
RANDOM_EVENTS = [
    ...
    RandomEvent(
        ...
        choices=[
            ...
            EventChoice("献上灵石", lambda p, g: gain_daoyun(10)(p, g) if p.lingshi >= 20 and (setattr(p, 'lingshi', p.lingshi - 20) or True) else "灵石不足", "risky"),
    ...
    RandomEvent(
        ...
        choices=[
            EventChoice("治疗他（消耗20%生命）", lambda p, g: (lose_health(0.2)(p, g), gain_daoyun(8))[1] if p.health > p.max_health * 0.3 else "生命不足", "risky"),
            EventChoice("给他丹药", lambda p, g: gain_lingshi(30)(p, g) if getattr(g, 'run_potions', 0) > 0 and (setattr(g, 'run_potions', g.run_potions - 1) or True) else "没有丹药", "safe"),
    ...
    RandomEvent(
        ...
        choices=[
            EventChoice("用50灵石换随机饰品", lambda p, g: gain_random_accessory()(p, g) if p.lingshi >= 50 and (setattr(p, 'lingshi', p.lingshi - 50) or True) else "灵石不足", "safe"),
            EventChoice("用30%生命换丹药", lambda p, g: (lose_health(0.3)(p, g), gain_potion(2))[1] if p.health > p.max_health * 0.4 else "生命不足", "risky"),
    ...
    RandomEvent(
        ...
        choices=[
            ...
            EventChoice("强行打开", lambda p, g: gain_lingshi(80)(p, g) if random.random() < 0.6 else lose_health(0.3)(p, g), "risky"),
    ...
    RandomEvent(
        ...
        choices=[
            EventChoice("献祭灵石（50）", lambda p, g: gain_random_accessory()(p, g) if p.lingshi >= 50 and (setattr(p, 'lingshi', p.lingshi - 50) or True) else "灵石不足", "safe"),
            EventChoice("献祭生命（30%）", lambda p, g: (lose_health(0.3)(p, g), max_health_boost(20))[1] if p.health > p.max_health * 0.4 else "生命不足", "risky"),
    ...
    RandomEvent(
        ...
        choices=[
            ...
            EventChoice("小赌怡情", lambda p, g: gain_lingshi(20)(p, g) if random.random() < 0.7 else lose_lingshi(0.3)(p, g), "risky"),
    ...
    RandomEvent(
        ...
        choices=[
            ...
            EventChoice("强行突破", lambda p, g: max_health_boost(30)(p, g) if random.random() < 0.5 else lose_health(0.4)(p, g), "dangerous"),
    ...
    RandomEvent(
        ...
        choices=[
            EventChoice("接受传承", lambda p, g: (max_health_boost(30)(p, g), max_mana_boost(30)(p, g), gain_random_accessory()(p, g))[2], "safe"),
