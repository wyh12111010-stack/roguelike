...
class UIComponent:
    ...
    def update(self, dt: float, mouse_pos: Tuple[int, int]) -> None:
        ...
        if self.visible:
            # 检查鼠标悬停
            mx, my = mouse_pos
            self.hover = (self.x <= mx <= self.x + self.width and 
...
class Panel(UIComponent):
    ...
    def _draw_impl(self, screen: pygame.Surface) -> None:
        ...
        pygame.draw.line(screen, self.border_color,
                        (self.x, self.y),
                        (self.x + corner, self.y), 3)
        # 右上角
        pygame.draw.line(screen, self.border_color,
                        (self.x + self.width - corner, self.y),
                        (self.x + self.width, self.y), 3)
        pygame.draw.line(screen, self.border_color,
                        (self.x + self.width, self.y),
                        (self.x + self.width, self.y + corner), 3)
        # 左下角
        pygame.draw.line(screen, self.border_color,
                        (self.x, self.y + self.height - corner),
                        (self.x, self.y + self.height), 3)
        pygame.draw.line(screen, self.border_color,
                        (self.x, self.y + self.height),
                        (self.x + corner, self.y + self.height), 3)
        # 右下角
        pygame.draw.line(screen, self.border_color,
